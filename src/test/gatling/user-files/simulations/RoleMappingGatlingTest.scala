...
/**
 * Performance test for the RoleMapping entity.
 */
class RoleMappingGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the RoleMapping entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all roleMappings")
            .get("/core/api/role-mappings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new roleMapping")
            .post("/core/api/role-mappings")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJSON
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created roleMapping")
                .get("/core${new_roleMapping_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created roleMapping")
            .delete("/core${new_roleMapping_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
